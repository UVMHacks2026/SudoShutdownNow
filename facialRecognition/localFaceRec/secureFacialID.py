...
def get_face_embedding(frame) -> Optional[np.ndarray]:
    try:
        ...
        if isinstance(frame, str):
            ...
            logger.error(f"Unsupported input type for frame: {type(frame)}")
            return None
...
if __name__ != "__main__":
    ...
    # DB init is skipped if DATABASE_URL is not explicitly set (e.g. local/test env).
    try:
        _db_url = os.getenv("DATABASE_URL", "")
        if _db_url and "user:password" not in _db_url:
            init_db()
            load_users()
        else:
            logger.warning("DATABASE_URL not configured — skipping DB init. Call init_db() manually.")
        init_face_app()
